#!/usr/bin/perl

use strict;
use warnings;
use feature 'state';
# use module
use XML::Simple;
use Data::Dumper;
use Lingua::EN::PluralToSingular 'to_singular';
use Lingua::EN::Sentence qw( get_sentences add_acronyms );
use Lingua::EN::Inflect::Number qw( to_S to_PL );
use open ":encoding(utf8)";
use open IN => ":encoding(utf8)", OUT => ":utf8";

our @pubmed_data;
our %word_data;
our %char_data;
# create object

my $xml = new XML::Simple (KeyAttr=>[]);


#remove_attribute($ARGV[0], "data1.txt");
my $data1 = $xml->XMLin("$ARGV[0]");
my $data2 = $xml->XMLin("$ARGV[1]");
#traverse( $data );


my $abstract1 = $data1->{PubmedArticle}->{MedlineCitation}->{Article}->{Abstract}->{AbstractText};
my $abstract2 = $data2->{PubmedArticle}->{MedlineCitation}->{Article}->{Abstract}->{AbstractText}->{content};
print "abstract1 = $abstract1\n";
print "abstract2 = $abstract2\n";
my $sentence1 = sentence($abstract1);
my $sentence2 = sentence($abstract2);
foreach (@$sentence2){
      print "sentence2 = $_\n";
  
  }

my $word1 = word(\@$sentence1);
my $word2 = word(\@$sentence2);
foreach (@$word1){
      print "word1 = $_\n";
  
  }

foreach (@$word2){
      print "word2 = $_\n";
  
  }
my %word1_hash;
my %word2_hash;
foreach (@$word1){
    if (exists $word1_hash{$_} ) {
                $word1_hash{$_}++;
            
            }
    else {
                $word1_hash{$_} = 1;
            }
    }
     foreach (sort keys %word1_hash) {
        print "key1 = $_ value1 = $word1_hash{$_}\n";

       
    }
foreach (@$word2){
    if (exists $word2_hash{$_} ) {
                $word2_hash{$_}++;
            
            }
    else {
                $word2_hash{$_} = 1;
            }
    }
     foreach (sort keys %word2_hash) {
        print "key2 = $_ value2 = $word2_hash{$_}\n";

       
    }
my @final_array;
foreach (sort keys %word1_hash){
    push @final_array, [1, $_, $word1_hash{$_}];
    
    
    }
foreach (sort keys %word2_hash){
    push @final_array, [2, $_, $word2_hash{$_}];
    
    
    }

print "---------\n";
print Data::Dumper->Dump([@final_array]);
my @sort_array = sort {$a->[1] cmp $b->[1]} @final_array;
open my $xml_file, '>', "data1.txt"
    or die "can't open $!";
foreach (@sort_array){
        printf $xml_file "%2d%20s%5d\n", $_->[0], $_->[1], $_->[2];
    
    }
close $xml_file;


#my char1 = char(@$word1);
#my char2 = char(@$word2);



sub sentence {
   add_acronyms('lt','gen');               ## adding support for 'Lt. Gen.'

 
   my $sentences=get_sentences($_[0]);     ## Get the sentences.
   foreach (@$sentences){
      print "sentencesss = $_\n";
  
  }
   return $sentences;   

}
sub word {
      my @word;
      #print"------------\n";
      #print Dumper @{$_[0]};
      
      foreach my $sentence (@{$_[0]}) {
        #print "test = $_";
        $sentence =~ s/(.*?),/$1/g;
        $sentence =~ s/\((.*?)\)/$1/g;
        $sentence =~ s/\[(.*?)\]/$1/g;
        $sentence =~ s/(.*):/$1/g;
        $sentence = lc $sentence;
        $sentence =~ s/(.*)(\.|\?|\:)$/$1/;
        
        my @word_array = split(/\s+/, $sentence);
        foreach (@word_array) {
            $_ =~ s/(.*):/$1/;
            $_ = to_S ($_);
            $_ =~ s/'(.*)'/$1/;
            push @word, $_;
         }
       
       }
   
       return \@word;
   }

=begin comment
sub traverse {
    our @pubmed_data=();
    my ($element) = @_;
    if( ref( $element ) =~ /HASH/ ) {
        foreach my $key (keys %$element) {
  #          print "key=$key\n";
  #          next if ($key eq "PubmedData");
            traverse( $$element{$key} );
            }
    } 
    elsif( ref( $element)  =~ /ARRAY/ )  {
        #my $i = $_[0];
        traverse( $_ ) foreach @$element;
    } 
    else {
   #     print "$element\n";
       push @pubmed_data, $element;
    }
}
sub remove_attribute {
    my $xml_file; # from $ARGV[0] == data.txt
    my $remove_attribute_file; #to data1.txt
    open $xml_file, '<', shift @_
        or die "can't open $!";

    open $remove_attribute_file, '>', shift @_;
        or die "can't open $!";
    while (<$xml_file>){
        $_ =~ m/<(.*?)>/;
        print "found $1\n";
        my $temp =$1;
        my $word;
        my @attribute = split(' ', $temp);
        print "attribute[0] = $attribute[0]\n";
     
        $word = $attribute[0];
        $_ =~ s/<(.*?)>/<$word>/;
        print $remove_attribute_file "$_";
    }
    close $remove_attribute_file;
    close $xml_file;

}

sub sentence {
    my $sentence_file; # to data2.txt
	%word_data = {};
    open $sentence_file, '>', shift @_
        or die "can't open $!";
    

    add_acronyms('lt','gen');               ## adding support for 'Lt. Gen.'

 
    foreach (@pubmed_data){
        state $sentence_count = 0;
    
        my $sentences=get_sentences($_);     ## Get the sentences.
        foreach my $sentence (@$sentences) {
            $sentence_count++;
            $sentence =~ s/(.*?),/$1/g;
            $sentence =~ s/\((.*?)\)/$1/g;
            $sentence =~ s/\[(.*?)\]/$1/g;
            $sentence =~ s/(.*):/$1/g;
            $sentence = lc $sentence;
    
            $sentence =~ s/(.*)(\.|\?|\:)$/$1/;
        
            print $sentence_file "$sentence_count.$sentence\n";
        
            my @word_array = split(/\s+/, $sentence);
            foreach (@word_array) {
            $_ =~ s/(.*):/$1/;
            $_ = to_S ($_);
            $_ =~ s/'(.*)'/$1/;
            if (exists $word_data{$_} ) {
                $word_data{$_}++;
            
            }
            else {
                $word_data{$_} = 1;
            }
        
           }        
        }
    
    }
    close $sentence_file;



}

sub word {
    my $word_file; #to data3.txt
    open $word_file, '>', shfit @_
        or die "can't open $!";
    foreach (sort keys %word_data) {
        print $word_file "key = $_\n";
        print $word_file "value =  $word_data{$_}\n";
       
    }
    close $word_file;
}

sub char {
    %char_data = {};
    my $char_file; #to data4.txt
    open $char_file, '>', shift @_
        or die "can't open $!";
    foreach (sort keys %word_data) {
        my @chars = split ("",$_);
        my $values = $word_data{$_};
        foreach (@chars) {
            if (exists $char_data{$_} ) {
                $char_data{$_} += $values;
            
            }
            else {
                $char_data{$_} = 1 * $values;
       
            }
     }
}
    foreach (sort keys %char_data) {
        print $char_file "key = $_\n";
        print $char_file "value =  $char_data{$_}\n";
       
    }
    close $char_file;
 
}

=end comment
=cut
